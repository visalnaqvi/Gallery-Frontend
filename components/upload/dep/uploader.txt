...
export default function FileUpload({ groupId, userId, onUploadStart, onUploadComplete }: FileUploadProps) {
    ...
    useEffect(() => {
        if (uploadState.isProcessing && !startTime) {
            ...
            onUploadStart();
        } else if (!uploadState.isProcessing && startTime) {
            setStartTime(null);
            onUploadComplete();
        }
    }, [uploadState.isProcessing, startTime, onUploadStart, onUploadComplete]);
